...
import { FormItemContext, FormItemContextKey, FormItemProps, FormItemRule, FormItemValidateState } from './formItem';
...
const validate: FormItemContext['validate'] = (trigger, callback) => {
    ...
    return validator.validate({ [modelName]: model[modelName] }).then(() => {
        onValidateSuccess()
    }).catch((err) => {
        onValidateError(err)
